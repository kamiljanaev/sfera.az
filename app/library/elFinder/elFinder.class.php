		...
		if (isset($_GET['init'])) {
			...
			if (isset($this->_commands['archive']) || isset($this->_commands['extract'])) {
				...
				if (isset($this->_commands['extract'])) {
                    if (array_key_exists('extract', $this->_options['archivers'])) {
...
//					$file = $dir.DIRECTORY_SEPARATOR.$_FILES['upload']['name'][$i];
					$aliasGenerator = new Core_Alias();
					$fileName = $aliasGenerator->generate($_FILES['upload']['name'][$i]);
	...
	protected function _checkName($n)
	{
		...
		$n = preg_match('|^[^\\/\<\>:]+$|', $n) ? $n : false;
		if ($n) {
			$n = $this->_formatName($n);
		}
		return $n;
	}
	
	protected function _formatName($n)
	{
		$toLowerFilter = new Zend_Filter_StringToLower;
		$toLowerFilter->setEncoding('UTF-8');

		$n = $toLowerFilter->filter($n);
		$n = $this->_translitName($n);
		$n = $this->_prepareName($n);
		return $n;
	}
	 ...
	 */
	protected function _translitName($n)
	{
	 ...
	 * @param string  $n - file name
	 */
	protected function _prepareName($n)
	{
		$result = preg_replace(
				array ('/[^0-9a-zA-Z_-]/', '/-+/', '/\+/'),
				array ('', ''),
				str_replace(' ', '_', strtr(utf8_decode(trim($n)), $this->_chars))
		);
		return $result;
	}
