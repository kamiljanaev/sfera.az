...
$(function(){
	var rowNumCookieKey = 'awards-row-num';
	...
	var dataGrid = $("#jqgridtable").jqGrid({
		...
		datatype: "json",
	   	colNames:['<?= $this->msgGridColumnTitle ?>', '<?= $this->msgGridColumnLogo ?>', '<?= $this->msgGridColumnStatus ?>', '<?= $this->msgGridColumnAction ?>'],
	   	colModel:[
	   		{name:'title',index:'title', width:270},
            {name:'logo',index:'logo', width:170, align:'center', search: false},
            {name:'status',index:'status', width:100, align:"center", stype:'select', editoptions:{value:'<?= $this->statusList ?>'}},
